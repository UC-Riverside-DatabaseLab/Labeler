...
from .models import Task, Participation,Document
...
#		print (temp)
				...
				task = form.create_task(task_title, task_description, self.request.user, prerequisite, task_num_labelers, task_num_posts, task_random_label, task_upload_file)
		#print (task)
...
#		task = get_object_or_404(Task, pk=self.kwargs['pk'])
	def dispatch(self, request):
		...
		if request.method == 'GET':
			...
			prime = UpdateLabelerForm(initial={'Select Task': request.GET.get('pk'), 'Participating Labelers':request.GET.get('clist')}) 			#UpdateLabelerForm(request.GET)
...
#		form = NameForm()
		...
		#def form_valid(self, form): 
			...
			#print(ttask)
				tcoders = form.cleaned_data['Participating Labelers']		
			#print(tcoders)
...
class TaskEvaluationDetailView(LoginRequiredMixin, DetailView):
	...
	def dispatch(self, request, *args, **kwargs):
		...
		self.coder_emails = PostResponse.objects.filter(task=self.task.pk).values_list('responder__email', flat=True).distinct().order_by('responder__email')
		post_list = self.task.post_list.all()
		if os.path.exists('/home/salae001/new/LabelingSystem-master/labelingsystem/media/csvfileFinal.csv'):
			print ('existsssss')
			os.remove('/home/salae001/new/LabelingSystem-master/labelingsystem/media/csvfileFinal.csv')
		if os.path.exists('/home/salae001/new/LabelingSystem-master/labelingsystem/media/csvfileP.csv'):
			os.remove('/home/salae001/new/LabelingSystem-master/labelingsystem/media/csvfileP.csv')
		if os.path.exists('/home/salae001/new/LabelingSystem-master/labelingsystem/media/csvfile.csv'):
			os.remove('/home/salae001/new/LabelingSystem-master/labelingsystem/media/csvfile.csv')
